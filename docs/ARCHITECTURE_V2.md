...
| `/v1/enterprise/identity/authorize` + `/v1/enterprise/identity/callback` | 企业 OIDC SSO 连接器骨架；authorize 返回 IdP 跳转 URL、callbackUrl、state/nonce 和接入 playbook；callback 目前接收 code/state 并明确后续 token exchange、ID Token 校验、邮箱域和组织成员绑定。 |
