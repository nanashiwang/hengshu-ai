...
企业身份策略已提供域名白名单、OIDC/SSO、SCIM 配置校验和接入 playbook；SCIM provision 已支持成员创建、查询、停用和 ListResponse；OIDC SSO 已具备 authorize 发起包、HMAC state 校验和 callback 组织上下文还原，真实 token exchange 与成员绑定仍是后续接入项。
...
| 真实 SSO 登录连接器        | 已有 OIDC authorize/callback 骨架与 HMAC state 校验；token exchange、ID Token 校验和成员绑定未接入 |
