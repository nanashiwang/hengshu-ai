...
当前代码中的 `SkillVersions` 已经具备 Skill Contract 的雏形：公开 Contract API 和详情页输出 hash、schema、权限、推荐模型、最低 Runner 版本、上一版本 diff 和客户复核 playbook，但不暴露 prompt 正文。
...
当前公开 ModelProfile API 已输出版本漂移、输入规模档/任务画像表现、回归告警、有效样本、来源权重和采用复验 checklist，并直达私人台账、失败库与 Adapter 排障入口。
...
当前 FailureCase 已支持“自动聚类 → 前台归因看板 → 人工归因/复验覆盖 → 私人台账复验计划 → 公开脱敏复核”的闭环；公开 API 输出脱敏画像、人工归因摘要、复验覆盖、triage checklist、私人台账复现入口、模型画像、Adapter、证据验签入口和登录后的 reverify-plan，不暴露原始输入输出。
...
企业身份策略已提供域名白名单、OIDC/SSO、SCIM 配置校验和接入 playbook；SCIM provision 已支持成员创建、查询、停用和 ListResponse；OIDC SSO 已具备 authorize 发起包、HMAC state 校验、callback 组织上下文还原和服务端 tokenExchange 请求包，真实 ID Token 校验与成员绑定仍是后续接入项。
...
| Skill 版本化     | 已有   | `SkillVersions`、`/v1/skills/[slug]/contract`，公开 Contract 摘要和复核 playbook |
...
| ModelProfile     | 已有   | `ModelProfiles`、`/v1/model-profiles`，公开漂移/输入规模档/任务画像/回归/样本来源和采用复验 playbook |
...
| FailureCase      | 已有   | `FailureCases`、`/v1/failures`、`/v1/failures/[id]/triage`、`/v1/failures/[id]/reverify-plan`、`/console/failures/triage`，公开脱敏画像、归因看板、人工归因摘要、复验覆盖、triage checklist、台账复现、自动复验计划和证据验签入口 |
...
| 企业身份策略       | 已有骨架 | `/v1/enterprise/identity`、`/v1/enterprise/identity/authorize`、`/v1/enterprise/identity/callback`、`/v1/enterprise/scim/users`，返回 SSO/SCIM 接入 playbook、OIDC authorize 发起包和 SCIM provision |
...
| 能力             | 当前状态                         | 需要补齐                            |
| -------------- | ---------------------------- | ------------------------------- |
| Skill Contract | 已有 hash、公开摘要、上一版本 diff、前台 diff 可视化/筛选和复核 playbook | 后续加强 diff 对比基线选择 |
| 兼容矩阵           | 有 `CompatReports`、ModelProfile、freshness、输入规模档/任务画像与采用复验 checklist | 后续细化更多 Skill 任务族画像 |
| 失败知识           | 已有一等 `FailureCase`、归因看板、人工归因、复验覆盖、Adapter 草稿链路、公开复验入口和私人台账 reverify-plan | 后续把计划接成批量队列           |
...
| 能力                   | 状态  |
| -------------------- | --- |
| 真实第三方签章 / 时间戳服务 | 已支持第三方发布声明和 receiptHash 本地复核；真实签章服务未接入 |
| 真实 SSO 登录连接器        | 已有 OIDC authorize/callback 骨架、HMAC state 校验和 tokenExchange 请求包；ID Token 校验和成员绑定未接入 |
| 更细粒度输入档漂移曲线       | 已按 inputBucket 与 taskProfile 输出表现摘要；任务族画像继续扩展 |
...
3. SSO / SCIM；
4. 私有部署；
5. 第三方可信声明与外锚复核 playbook；
6. 外部时间戳 receiptHash 本地复核（真实签章服务后续接入）；
7. 可信网络；
