...
当前公开 ModelProfile API 已输出版本漂移、输入规模档表现、回归告警、有效样本、来源权重和采用复验 checklist，并直达私人台账、失败库与 Adapter 排障入口。
...
| ModelProfile     | 已有   | `ModelProfiles`、`/v1/model-profiles`，公开漂移/输入规模档/回归/样本来源和采用复验 playbook |
...
| 兼容矩阵           | 有 `CompatReports`、ModelProfile、freshness、输入规模档画像与采用复验 checklist | 后续细化更多任务 profile 画像 |
...
| 真实 SSO 登录连接器        | 未接入 |
| 更细粒度输入档漂移曲线       | 已按 inputBucket 输出表现摘要；任务 profile 继续扩展 |
