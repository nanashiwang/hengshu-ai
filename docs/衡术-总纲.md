...
企业 Registry 已在批准时冻结 Contract/Passport/证书采用基线，用于后续版本漂移和重新审批判断；企业身份策略已提供域名白名单、OIDC/SSO、SCIM 配置校验和接入 playbook；SCIM provision 已支持成员创建、查询、停用和 ListResponse；OIDC SSO 已具备 authorize 发起包、HMAC state 校验、callback 组织上下文还原、服务端 tokenExchange 请求包、ID Token claims 校验和 active 成员绑定预检；JWKS 签名校验与正式会话签发仍是后续接入项。
...
| 能力             | 当前状态                         | 需要补齐                            |
| -------------- | ---------------------------- | ------------------------------- |
| Skill Contract | 已有 hash、公开摘要、可选基线 diff、前台 diff 可视化/筛选、复核 playbook 和企业批准时采用基线记录 | 后续加强基线漂移告警 |
