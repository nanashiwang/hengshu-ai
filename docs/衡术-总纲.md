...
当前 Adapter 已支持“FailureCase → 待人工评审草稿 → 前台评审看板 → 审核批准后 active 复用”的闭环；公开 API 只输出已批准 active Adapter 的 lift 摘要、复用/复验 checklist、私人台账复验入口、模型画像入口和证据验签入口；补丁正文仍只对作者、审核员或管理员开放。
...
当前 FailureCase 已支持“自动聚类 → 前台归因看板 → 人工归因/复验覆盖 → 私人台账复验计划 → 批量复验队列 → 自动复验 worker 回写覆盖 → 公开脱敏复核”的闭环；公开 API 输出脱敏画像、人工归因摘要、复验覆盖、triage checklist、私人台账复现入口、模型画像、Adapter、证据验签入口和登录后的 reverify-plan；登录用户可把计划放入 reverify-queue，按 failureCaseId+userId 去重，worker 只记录运行 ID、成功率和格式率，不暴露原始输入输出。
...
| AdapterProfile   | 已有   | `AdapterProfiles`、`/v1/adapters`、`/console/adapters/review`，待评审草稿、前台评审看板、审核批准门禁、公开 lift、复用/复验 checklist、台账复验和证据验签入口 |
...
| 失败知识           | 已有一等 `FailureCase`、归因看板、人工归因、复验覆盖、Adapter 草稿链路、公开复验入口、私人台账 reverify-plan、Redis 批量入队接口和自动复验 worker 回写 | 后续补更细的复验调度、失败重试和审核员批量确认 |
...
| Adapter 人工评审工作流增强   | 已支持 reviewStatus 门禁和前台评审看板；后续补批量评审/复验自动化 |
