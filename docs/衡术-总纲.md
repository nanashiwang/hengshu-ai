...
当前代码中的 `SkillVersions` 已经具备 Skill Contract 的雏形：公开 Contract API 输出 hash、schema、权限、推荐模型、最低 Runner 版本、上一版本 diff 和客户复核 playbook，但不暴露 prompt 正文。
...
当前公开 Adapter API 已输出 lift 摘要、复用/复验 checklist、私人台账复验入口、模型画像入口和证据验签入口；补丁正文仍只对作者、审核员或管理员开放。
...
| AdapterProfile   | 已有   | `AdapterProfiles`、`/v1/adapters`，公开 lift、复用/复验 checklist、台账复验和证据验签入口 |
...
| 能力             | 当前状态                         | 需要补齐                            |
| -------------- | ---------------------------- | ------------------------------- |
| Skill Contract | 已有 hash、公开摘要、上一版本 diff 和复核 playbook | 后续加强前台 diff 可视化 |
