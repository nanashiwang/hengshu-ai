...
> 衡术不是让你重买一套模型，而是帮你把 AI Skill 放进已有模型和企业环境里，先看 Passport 和验签证据，再试跑，失败可追踪，换模型可复验，企业可审批和审计。
...
当前公开 ModelProfile API 已输出版本漂移、输入规模档/任务画像/Skill任务画像表现、回归告警、有效样本、来源权重和采用复验 checklist，并直达私人台账、失败库与 Adapter 排障入口。
...
当前 Adapter 已支持“FailureCase → 待人工评审草稿 → 前台评审看板/批量评审 → 审核批准后 active 复用”的闭环；公开 API 只输出已批准 active Adapter 的 lift 摘要、复用/复验 checklist、私人台账复验入口、模型画像入口和证据验签入口；补丁正文仍只对作者、审核员或管理员开放。
...
当前 FailureCase 已支持“自动聚类 → 前台归因看板 → 人工归因/复验覆盖 → 私人台账复验计划 → 批量复验队列 → 自动复验 worker 有限重试并回写覆盖 → 公开脱敏复核”的闭环；公开 API 输出脱敏画像、人工归因摘要、复验覆盖、triage checklist、私人台账复现入口、模型画像、Adapter、证据验签入口和登录后的 reverify-plan；登录用户可把计划放入 reverify-queue，按 failureCaseId+userId 去重，worker 只记录运行 ID、成功率和格式率，不暴露原始输入输出。
...
当前企业 Registry 已返回治理 checklist/playbook，串起组织内 Passport/证书复核、模型白名单、企业运行授权、审计导出和组织内失败库。

企业 Registry 已在批准时冻结 Contract/Passport/证书采用基线，并提供批量重审入口：集中列出 Contract、版本、Passport、证书相对采用基线的漂移项，支持批量刷新基线、标记已复核或接受风险；企业身份策略已提供域名白名单、OIDC/SSO、SCIM 配置校验和接入 playbook；SCIM provision 已支持成员创建、查询、停用和 ListResponse；OIDC SSO 已具备 authorize 发起包、HMAC state 校验、callback 组织上下文还原、服务端 tokenExchange 请求包、ID Token claims 校验、JWKS RS256 签名校验和 active 成员绑定预检；正式会话签发仍是后续接入项。
...
| 必备 Skill onboarding | 已有   | `/v1/skills?essential=1` 返回推荐理由和“看 Passport → 默认输入试跑 → 回台账重跑”starterPlaybook |
...
| 本地 Runner     | 已有   | `runner/hengshu.mjs`、`/v1/runner/install`、`/v1/runner/check`，安装/更新返回验签/本地跑/脱敏回流/复验 playbook |
| 运行台账          | 已有   | `SkillRuns`                              |
| 输入输出加密        | 已有   | `SkillRuns` hooks                        |
| 兼容报告          | 已有   | `CompatReports`                          |
| ModelProfile     | 已有   | `ModelProfiles`、`/v1/model-profiles`，公开漂移/输入规模档/任务画像/Skill任务画像/回归/样本来源和采用复验 playbook |
| AdapterProfile   | 已有   | `AdapterProfiles`、`/v1/adapters`、`/console/adapters/review`、`/v1/adapters/review`，待评审草稿、前台评审看板、批量评审、审核批准门禁、公开 lift、复用/复验 checklist、台账复验和证据验签入口 |
| FailureCase      | 已有   | `FailureCases`、`/v1/failures`、`/v1/failures/[id]/triage`、`/v1/failures/[id]/reverify-plan`、`/v1/failures/[id]/reverify-queue`、`worker:reverify-queue`、`/console/failures/triage`，公开脱敏画像、归因看板、人工归因摘要、复验覆盖、triage checklist、台账复现、自动复验计划、批量复验入队、worker 结果回写和证据验签入口 |
| Enterprise Registry | 已有 | `EnterpriseRegistries`、`/v1/enterprise/registry/[id]/passport`、`/v1/enterprise/registry/review-required`，返回治理 checklist、组织内证书/Passport、采用基线漂移告警、批量重审、审计导出和企业失败库入口 |
| 企业身份策略       | 已有骨架 | `/v1/enterprise/identity`、`/v1/enterprise/identity/authorize`、`/v1/enterprise/identity/callback`、`/v1/enterprise/scim/users`，返回 SSO/SCIM 接入 playbook、OIDC authorize 发起包、ID Token claims 校验/成员绑定预检和 SCIM provision |
| 贡献与 credit 台账 | 已有骨架 | `ContributionLogs`、`CreditLogs`          |
| New API 联动    | 已有骨架 | `newapiAdmin`、`DeploymentSettings`       |
| 审计日志          | 已有   | `AuditLogs`                              |
| AI 审核         | 已有   | Skill 合规审核员 + 上传流程                       |
| Skill Passport  | 已有   | `SkillPassports`、`/v1/skills/[slug]/passport`，公开脱敏摘要、验签入口和复核 playbook |
...
| 兼容矩阵           | 有 `CompatReports`、ModelProfile、freshness、输入规模档/任务画像与采用复验 checklist | 后续细化更多 Skill 任务族画像 |
| 失败知识           | 已有一等 `FailureCase`、归因看板、人工归因、复验覆盖、Adapter 草稿链路、公开复验入口、私人台账 reverify-plan、Redis 批量入队接口和自动复验 worker 回写和失败有限重试 | 后续补更细的复验调度、失败重试和审核员批量确认 |
| 路由策略           | 已有 routePolicy               | 降级为适配 / 优化能力                    |
| 企业使用           | 已有 Registry、Org、Policy、Approval、采用基线、批量重审、身份策略和 SCIM provision 骨架 | 后续接真实 SSO 登录连接器和控制台治理总览 |

### 4.3 未落地能力

| 能力                   | 状态  |
| -------------------- | --- |
| 真实第三方签章 / 时间戳服务 | 已支持第三方发布声明、receiptHash 本地复核和第三方时间戳 imprint 请求包；真实签章服务未接入 |
| 真实 SSO 登录连接器        | 已有 OIDC authorize/callback 骨架、HMAC state 校验、tokenExchange 请求包、ID Token claims 校验、JWKS RS256 签名校验和成员绑定预检；正式会话签发未接入 |
| 更细粒度输入档漂移曲线       | 已按 inputBucket、taskProfile 与 skillProfile 输出表现摘要；任务族画像继续扩展 |
| Adapter 人工评审工作流增强   | 已支持 reviewStatus 门禁、前台评审看板和批量评审；后续补复验自动化 |
...
单纯“攒兼容数据”不是护城河：结论密度低、容易被爬、样本量本身不等于可信、模型网关在数据上游、数据还会腐坏。衡术的护城河必须长在真实运行、私人台账、失败复验和企业治理里。
...
| 2 私人台账 | 用户自己的运行历史、输入输出、重跑血缘和换模型复验 | `SkillRuns`、`/console/runs`、`/v1/runs`、reverify-plan | 这是用户私有资产；用得越多，迁移成本越高，也堵住 BYOK 查完就走 |
| 3 任务可靠性 | 按 Skill、模型、输入档、错误类型沉淀失败和修复 | `FailureCases`、`AdapterProfiles`、归因看板、复验覆盖 | 模型趋同后，任务/Prompt/输出契约差异仍存在，可靠性经验反而更值钱 |
| 4 可验签基准 | Passport、证书、证据快照、外锚和黄金样例逐条打分 | `/verify`、`/v1/evidence/verify`、`/v1/anchors/verify`、certificate API | 网关自评有利益冲突；中立证据链、签名和第三方时间戳更适合采购/审计 |
...
6. 外部时间戳 imprint 请求包 + receiptHash 本地复核（真实签章服务后续接入）；
7. 可信网络；
