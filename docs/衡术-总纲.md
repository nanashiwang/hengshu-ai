...
当前 FailureCase 已支持“自动聚类 → 前台归因看板 → 人工归因/复验覆盖 → 公开脱敏复核”的闭环；公开 API 输出脱敏画像、人工归因摘要、复验覆盖、triage checklist、私人台账复现入口、模型画像、Adapter 和证据验签入口，不暴露原始输入输出。
...
| FailureCase      | 已有   | `FailureCases`、`/v1/failures`、`/v1/failures/[id]/triage`、`/console/failures/triage`，公开脱敏画像、归因看板、人工归因摘要、复验覆盖、triage checklist、台账复现和证据验签入口 |
...
| 失败知识           | 已有一等 `FailureCase`、归因看板、人工归因、复验覆盖、Adapter 草稿链路和公开复验入口 | 后续加强自动复验           |
