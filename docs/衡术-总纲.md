...
当前代码中的 `SkillVersions` 已经具备 Skill Contract 的雏形：公开 Contract API 输出 hash、schema、权限、推荐模型、最低 Runner 版本和客户复核 playbook，但不暴露 prompt 正文。
...
| 能力             | 当前状态                         | 需要补齐                            |
| -------------- | ---------------------------- | ------------------------------- |
| Skill Contract | 已有 hash、公开摘要和复核 playbook       | 后续加强 Contract diff 可视化          |
