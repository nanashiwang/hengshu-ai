...
企业身份策略已提供域名白名单、OIDC/SSO、SCIM 配置校验和接入 playbook；SCIM provision 已支持成员创建、查询、停用和 ListResponse，真实 SSO 登录连接器仍是后续接入项。
...
| 企业身份策略       | 已有骨架 | `/v1/enterprise/identity`、`/v1/enterprise/scim/users`，返回 SSO/SCIM 接入 playbook，支持 SCIM provision |
