...
* 平台获得脱敏兼容数据；
* Skill 可以跨模型环境复用；
* 安装 API 和 Runner 会提示“验签 → 本地运行 → 脱敏回流 → 更新”的可信闭环。
...
| 本地 Runner     | 已有   | `runner/hengshu.mjs`、`/v1/runner/install`，安装返回验签/本地跑/脱敏回流/更新 playbook |
| 运行台账          | 已有   | `SkillRuns`                              |
...
| 能力             | 当前状态                         | 需要补齐                            |
| -------------- | ---------------------------- | ------------------------------- |
| Skill Contract | `SkillVersions` 有字段，但语义还不够强  | 明确 contract 定义                  |
| 兼容矩阵           | 有 `CompatReports` 聚合         | 需要 ModelProfile 和 freshness     |
| 失败知识           | 有雏形                          | 需要一等 `FailureCase` 集合           |
| 路由策略           | 已有 routePolicy               | 降级为适配 / 优化能力                    |
| 企业使用           | 有 `visibility=enterprise` 意识 | 需要 Registry、Org、Policy、Approval |
...
| `AdapterProfile`     | 未落地 |
| `CompatTestCase`     | 未落地 |
| `FailureCase` 一等集合   | 未落地 |
| `EnterpriseRegistry` | 未落地 |
| Passport 签名快照        | 未落地 |
| 企业策略包                | 未落地 |
| 组织 / 团队 / 租户隔离       | 未落地 |
