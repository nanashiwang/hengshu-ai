...
当前公开 FailureCase API 已输出脱敏画像、triage checklist、私人台账复现入口、模型画像、Adapter 和证据验签入口，不暴露原始输入输出。
...
| FailureCase      | 已有   | `FailureCases`、`/v1/failures`，公开脱敏画像、triage checklist、台账复现和证据验签入口 |
...
| 失败知识           | 已有一等 `FailureCase`、Adapter 草稿链路和公开复验入口 | 后续加强人工归因与复验覆盖           |
