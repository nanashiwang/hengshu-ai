...
企业 Registry 已在批准时冻结 Contract/Passport/证书采用基线，并在组织内 Passport 复核时输出基线漂移告警，用于后续版本漂移和重新审批判断；企业身份策略已提供域名白名单、OIDC/SSO、SCIM 配置校验和接入 playbook；SCIM provision 已支持成员创建、查询、停用和 ListResponse；OIDC SSO 已具备 authorize 发起包、HMAC state 校验、callback 组织上下文还原、服务端 tokenExchange 请求包、ID Token claims 校验和 active 成员绑定预检；JWKS 签名校验与正式会话签发仍是后续接入项。
...
| Enterprise Registry | 已有 | `EnterpriseRegistries`、`/v1/enterprise/registry/[id]/passport`，返回治理 checklist、组织内证书/Passport、审计导出和企业失败库入口 |
...
| 能力             | 当前状态                         | 需要补齐                            |
| -------------- | ---------------------------- | ------------------------------- |
| Skill Contract | 已有 hash、公开摘要、可选基线 diff、前台 diff 可视化/筛选、复核 playbook、企业批准时采用基线记录和基线漂移告警 | 后续加强批量重审流程 |
...
| 企业使用           | 已有 Registry、Org、Policy、Approval、身份策略和 SCIM provision 骨架 | 后续接真实 SSO 登录连接器 |
...
| 真实 SSO 登录连接器        | 已有 OIDC authorize/callback 骨架、HMAC state 校验、tokenExchange 请求包、ID Token claims 校验和成员绑定预检；JWKS 签名校验与正式会话签发未接入 |
