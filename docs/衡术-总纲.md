...
当前 Adapter 已支持“FailureCase → 待人工评审草稿 → 审核批准后 active 复用”的闭环；公开 API 只输出已批准 active Adapter 的 lift 摘要、复用/复验 checklist、私人台账复验入口、模型画像入口和证据验签入口；补丁正文仍只对作者、审核员或管理员开放。
...
| AdapterProfile   | 已有   | `AdapterProfiles`、`/v1/adapters`，待评审草稿、审核批准门禁、公开 lift、复用/复验 checklist、台账复验和证据验签入口 |
...
| Adapter 人工评审工作流增强   | 已支持 reviewStatus 门禁；后续补前台评审看板 |
