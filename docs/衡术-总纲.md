...
当前公开 ModelProfile API 已输出版本漂移、回归告警、有效样本、来源权重和采用复验 checklist，并直达私人台账、失败库与 Adapter 排障入口。
...
| ModelProfile     | 已有   | `ModelProfiles`、`/v1/model-profiles`，公开漂移/回归/样本来源和采用复验 playbook |
...
| 兼容矩阵           | 有 `CompatReports`、ModelProfile、freshness 与采用复验 checklist | 后续细化更多输入档画像              |
