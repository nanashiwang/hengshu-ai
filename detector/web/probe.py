...
def _prune_cache(now: float, *, reserve: int = 0) -> None:
    """Expire old probes and cap memory under high-cardinality traffic."""
    expired = [key for key, (created, _) in _CACHE.items() if now - created >= CACHE_TTL_S]
    for key in expired:
        _CACHE.pop(key, None)
    overflow = len(_CACHE) + reserve - MAX_CACHE_ENTRIES
    if overflow > 0:
        oldest = sorted(_CACHE, key=lambda key: _CACHE[key][0])
...
async def probe_relay(base_url: str, api_key: str) -> dict[str, Any]:
    ...
    now = time.monotonic()
    _prune_cache(now, reserve=0 if key in _CACHE else 1)
    ...
    try:
        async with httpx.AsyncClient(
            timeout=PROBE_TIMEOUT_S, trust_env=False
        ) as client:
    ...
    if status in (401, 403):
        out = _err(
            f"鉴权失败 (HTTP {status}): {_excerpt(resp.text, secret=api_key)}",
            base_url,
            status=status,
            auth_ok=False,
        )
    ...
    if status >= 400:
        out = _err(
            f"HTTP {status}: {_excerpt(resp.text, secret=api_key)}",
            base_url,
            status=status,
        )
        _CACHE[key] = (now, out)
...
def _excerpt(text: str, n: int = 160, *, secret: str = "") -> str:
    s = redact_text((text or "").strip().replace("\n", " "), secret)
...
async def probe_model_alive(
    ...
    try:
        ...
        if isinstance(body, str) and body:
            return False, _excerpt(body, 280, secret=api_key)
        return False, f"{type(e).__name__}: {_excerpt(str(e), 280, secret=api_key)}"
