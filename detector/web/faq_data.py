...
FAQ_CATEGORIES: tuple[FAQCategory, ...] = (
    FAQCategory(
        ...
        entries=(
            ...
            FAQEntry(
                ...
                "甚至注入额外 system prompt。格物 检测的就是这种「中间到底动了什么」。",
            ),
            FAQEntry(
                "relay-legal",
                "中转站合法吗?用了会不会出问题?",
            ...
            FAQEntry(
                "relay-steal-key",
                "用中转站的 API key 有什么风险?",
                "格物 实测过很多家中转站,常见的坑有 5 种(都见过真实案例):"
                ...
                "你真去用其中一些,直接报「没有这个模型」。格物 在你提交检测前会先帮你"
    ...
    FAQCategory(
        ...
        entries=(
            ...
            FAQEntry(
                ...
                "让模型说「我是 Claude」。格物 把「身份一致性」检测的权重设为 5%,"
            ...
            FAQEntry(
                ...
                "格物 在 thinking_signature 检测上会直接判 0 分,fail 整体检测。",
            ),
            FAQEntry(
                "gpt-becomes-claude",
                ...
                "实际后端模型。格物 把这种协议不兼容风险标记为 critical 级别。",
            ),
        ),
    ),
    FAQCategory(
        ...
        intro=(
            "Claude 是 格物 检测最深入的协议 — thinking block 可以做原始回放与篡改"
        ...
        entries=(
            FAQEntry(
                ...
                "thinking signature 是 Claude 启用扩展思考时返回的 opaque 字段。格物 不只看"
            ...
            FAQEntry(
                "claude-code-relay",
                "Claude code 中转站怎么挑?",
                "关键看 4 点:① 跑一次 格物 看 thinking signature 是否拿到 100 分;"
            ...
            FAQEntry(
                ...
                "格物 的 thinking_signature detector 已经针对这个切到非流式,确保检测可靠。",
            ),
            FAQEntry(
                "claude-cost",
                ...
                "Opus ~$0.20。格物 自己不收费 — 你付的是上游 API 的真实 token 钱。",
            ),
            FAQEntry(
                "claude-zero-score",
                "检测出 0 分是不是这家中转站完全没用?",
    ...
    FAQCategory(
        ...
        title="OpenAI 中转站",
        intro=(
            "OpenAI 协议没有同类 thinking block 签名回放信号,格物 在 OpenAI 上主要做"
        ...
        entries=(
            FAQEntry(
                ...
                "会强烈提示异源协议适配或自定义 usage 扩展。格物 标为 critical 并让该项"
            ...
            FAQEntry(
                ...
                "(尤其 protocol detector 报多个 critical),格物 会把 verdict 砍到 marginal — "
            ...
            FAQEntry(
                ...
                "格物 的预探测会列出该 key 在中转站上有多少模型属于本协议。"
    ...
    FAQCategory(
        ...
        intro=(
            "格物 的 Gemini 检测只走 OpenAI 兼容路径(POST /chat/completions),"
        ...
        entries=(
            ...
            FAQEntry(
                ...
                "finish_reason=length。格物 的 detector 已经把 max 调到 64-384。",
            ),
            FAQEntry(
                "gemini-no-native",
                "格物 为什么不支持 Gemini 原生 /v1beta/models/X:generateContent 路径?",
                "99% 的第三方 Gemini 中转站只暴露 OpenAI 兼容协议,Google 官方也提供 OpenAI 兼容"
                "端点 /v1beta/openai。维护两套独立的检测逻辑成本高且容易引入翻译层导致结果失真,"
                "所以 格物 集中在 OpenAI 兼容路径上做透。",
            ),
            FAQEntry(
                "gemini-pick",
                "Gemini 中转站怎么挑?",
            ...
            FAQEntry(
                ...
                "Google 官方 Gemini API 也能用 格物 测吗?",
    ...
    FAQCategory(
        ...
        title="工具对比",
        intro=(
            "中转站检测领域不止 格物 一家,这一组讲清楚 格物 跟同类工具的差异、"
            "以及怎么挑中转站。"
        ),
        entries=(
            FAQEntry(
            ...
            FAQEntry(
                ...
                "格物 在 Claude 上有 thinking block 原始/篡改回放验证、"
                "OpenAI 上有协议转换指纹识别(usage_source 等 critical 级)、"
                "Gemini 上适配 thinking-by-default 模型,深度更够。",
            ),
            FAQEntry(
                "vs-leaderboards",
                "中转站对比榜单(aiapipk.com / 知乎评测)跟 格物 是什么关系?",
                "那些是「主观评测」或「广告导向」,格物 是「客观技术检测」。"
                "可以互补:用榜单初筛信誉好的中转站,再用 格物 做技术验证。",
            ),
            FAQEntry(
                "how-to-pick-relay",
                "怎么挑中转站?有什么硬性指标?",
                "格物 推荐 5 条硬指标:① thinking signature ≥ 100;"
