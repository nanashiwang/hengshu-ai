...
FAQ_CATEGORIES: tuple[FAQCategory, ...] = (
    FAQCategory(
        ...
        entries=(
            ...
            FAQEntry(
                ...
                "甚至注入额外 system prompt。溯源 检测的就是这种「中间到底动了什么」。",
            ),
            FAQEntry(
                "relay-legal",
                "中转站合法吗?用了会不会出问题?",
            ...
            FAQEntry(
                "relay-steal-key",
                "用中转站的 API key 有什么风险?",
                "溯源 实测过很多家中转站,常见的坑有 5 种(都见过真实案例):"
                ...
                "你真去用其中一些,直接报「没有这个模型」。溯源 在你提交检测前会先帮你"
    ...
    FAQCategory(
        ...
        entries=(
            ...
            FAQEntry(
                ...
                "让模型说「我是 Claude」。溯源 把「身份一致性」检测的权重设为 5%,"
            ...
            FAQEntry(
                ...
                "溯源 在 thinking_signature 检测上会直接判 0 分,fail 整体检测。",
            ),
            FAQEntry(
                "gpt-becomes-claude",
                ...
                "实际后端模型。溯源 把这种协议不兼容风险标记为 critical 级别。",
            ),
        ),
    ),
    FAQCategory(
        ...
        intro=(
            "Claude 是 溯源 检测最深入的协议 — thinking block 可以做原始回放与篡改"
            "对照验证,提供比字段存在性更强的协议证据。"
        ),
        entries=(
            FAQEntry(
                "thinking-signature",
                "thinking signature 是什么?溯源 怎么验证它?",
                "thinking signature 是 Claude 启用扩展思考时返回的 opaque 字段。溯源 不只看"
            ...
            FAQEntry(
                "claude-code-relay",
                "Claude code 中转站怎么挑?",
                "关键看 4 点:① 跑一次 溯源 看 thinking signature 是否拿到 100 分;"
            ...
            FAQEntry(
                ...
                "溯源 的 thinking_signature detector 已经针对这个切到非流式,确保检测可靠。",
            ),
            FAQEntry(
                "claude-cost",
                ...
                "Opus ~$0.20。溯源 自己不收费 — 你付的是上游 API 的真实 token 钱。",
            ),
            FAQEntry(
                "claude-zero-score",
                "检测出 0 分是不是这家中转站完全没用?",
    ...
    FAQCategory(
        ...
        title="OpenAI 中转站",
        intro=(
            "OpenAI 协议没有同类 thinking block 签名回放信号,溯源 在 OpenAI 上主要做"
        ...
        entries=(
            FAQEntry(
                ...
                "会强烈提示异源协议适配或自定义 usage 扩展。溯源 标为 critical 并让该项"
            ...
            FAQEntry(
                ...
                "(尤其 protocol detector 报多个 critical),溯源 会把 verdict 砍到 marginal — "
            ...
            FAQEntry(
                ...
                "溯源 的预探测会列出该 key 在中转站上有多少模型属于本协议。"
    ...
    FAQCategory(
        ...
        intro=(
            "溯源 的 Gemini 检测只走 OpenAI 兼容路径(POST /chat/completions),"
        ...
        entries=(
            FAQEntry(
                ...
                "溯源 提交前会做 preflight,500ms 内识别死模型并给出可用列表替代。",
            ),
            FAQEntry(
    ...
    FAQCategory(
        ...
        entries=(
            ...
            FAQEntry(
                "self-host",
                "可以本地自托管 溯源 吗?",
                "可以,溯源 完全开源。clone 仓库 → pip install -e .[web] → "
                "uvicorn web.server:app 即可。CLI 也可以单独用:suyuan detect "
    ...
    FAQCategory(
        ...
        title="隐私与安全",
        intro=(
            "把 API key 交给一个检测工具是有风险的事,所以 溯源 在数据处理上的承诺需要"
        ...
        entries=(
            ...
            FAQEntry(
                ...
                "cctest.ai 也声称不上传 key,但代码闭源无法验证。溯源 完全开源,"
    ...
    FAQCategory(
        ...
        title="工具对比",
        intro=(
            "中转站检测领域不止 溯源 一家,这一组讲清楚 溯源 跟同类工具的差异、"
            "以及怎么挑中转站。"
        ),
        entries=(
            FAQEntry(
                "vs-cctest",
                "溯源 跟 cctest.ai 有什么区别?",
                "cctest 只做 Claude(单协议),用「黑盒检测」对抗规避,但维度有限。"
                "溯源 三协议(Claude / OpenAI / Gemini)+ thinking block 签名回放验证 + "
            ...
            FAQEntry(
                ...
                "溯源 在 Claude 上有 thinking block 原始/篡改回放验证、"
                "OpenAI 上有协议转换指纹识别(usage_source 等 critical 级)、"
                "Gemini 上适配 thinking-by-default 模型,深度更够。",
            ),
            FAQEntry(
                "vs-leaderboards",
                "中转站对比榜单(aiapipk.com / 知乎评测)跟 溯源 是什么关系?",
                "那些是「主观评测」或「广告导向」,溯源 是「客观技术检测」。"
                "可以互补:用榜单初筛信誉好的中转站,再用 溯源 做技术验证。",
            ),
            FAQEntry(
                "how-to-pick-relay",
                "怎么挑中转站?有什么硬性指标?",
                "溯源 推荐 5 条硬指标:① thinking signature ≥ 100;"
