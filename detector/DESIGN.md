...
> **版本说明**：本文档的协议字段、事件名和模型配置应以对应厂商当前官方文档为准（来源见附录 D）。标注 ⭐ 的检测项代表当前实现中的高强度协议证据,不等于本地公钥验签或绝对模型身份证明。
...
10 项检测，按目的分三组。**权重已根据证据强度调整** —— 原始/篡改 thinking block 回放信号权重最高，可被 system prompt 注入伪造的（identity）权重最低。
...
**权重设计原则**：越难伪造的特征权重越高。`signature_delta` 本身是不透明字符串,只看存在性和长度不构成证明;检测器必须同时验证原始 block 可回放、篡改单字节的 block 被官方形状错误拒绝,因此该组合证据权重最高。
...
### 3.3 ThinkingSignatureDetector（思维签名验证）⭐ 原始/篡改双向回放

**原理**（核心创新）：

Claude 4.5+ 系列在 extended thinking / adaptive thinking 模式下，thinking 块可包含 opaque `signature`。检测器不能在本地解析或公钥验签，因此必须验证原始 block 能回放，并用篡改单字符的 block 做负向对照；仅凭字段存在或长度不构成证明。
...
2. 发起 non-streaming thinking 请求,prompt 使用非官方示例的多步推理题,降低针对固定探针做特殊路由的价值。non-streaming 用于稳定拿到完整 thinking block。

3. 解析响应,检查 `thinking` block 的 `signature` 或 `redacted_thinking` block 的 `data`。字段存在和长度只用于决定能否继续回放,不作为来源证明。

4. 强制执行正向和负向对照：
   - 正向:把原始 assistant content 原样带入下一轮,必须被接受。
   - 负向:复制同一 content,只修改 `signature` / `data` 的一个字符,必须返回 400,且错误文本明确说明 thinking block 不得修改。
   - 两项同时满足才给 100 分。任意未知原因的拒绝都不能冒充成功验证。

**评分**：
- 100：原始 block 回放成功,篡改 block 被符合文档形状的 400 thinking 校验错误拒绝
- 65：篡改 block 被拒绝,但拒绝原因无法验证
- 60：字段存在但篡改 block 被接受或无法形成有效负向对照
- 50：原始 block 回放失败
- 40：opaque 字段异常短
- 20：有 thinking block 但没有 opaque 字段
- 0：未出现 thinking block

**适用范围**：仅 Opus 4.7 / Sonnet 4.6 / Haiku 4.5（其他模型 → skip，不计入分母）。

**为什么权重最高**：它同时验证服务端接受原始 block、拒绝最小篡改,比字段存在性强得多。但中转层仍可能专门代理探针请求,因此它不是本地公钥验签,不能表述为绝对真伪证明。
...
|---|---|---|---|---|---|
| **quick**（默认） | `--mode quick` | 5 项核心 | 约 8 | ~15s | 依模型与实际输出而定 |
| standard | `--mode standard` | 8 项 | 约 14 | ~40s | 依模型与实际输出而定 |
| full | `--mode full` | 全部 10 项 | 约 16-18 | ~70s | 依模型与实际输出而定 |
...
**为什么 quick 模式有用**：ThinkingSignature 原始/篡改回放是 10 项里的高强度协议指标（25% 权重），加上 Identity（5%）+ Consistency（10%）+ Protocol（5%）+ MessageID（5%），合计 50% 权重可快速给出协议透传与风险的初步判断。需要更完整结论时使用 standard / full。
...
4. **成本与性能**（实际值会随模型、thinking 输出和中转站计价变化）：
   - quick：约 8 请求 / ~15s
   - standard：约 14 请求 / ~40s
   - full：约 16-18 请求 / ~70s
   - 报告以客户端实际统计的请求数和 token 用量为准。
...
7. **Signature 验证局限**：当前实现强制执行原始/篡改回放,但客户端没有公钥且不能本地验证 opaque 字段。恶意中转层仍可能识别并专门代理探针请求,所以结论必须表述为高强度协议证据,不是绝对身份认证。
