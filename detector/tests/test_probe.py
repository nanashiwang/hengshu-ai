...
def test_probe_cache_expires_and_caps_high_cardinality(monkeypatch):
    import web.probe as probe

    probe.clear_cache()
    monkeypatch.setattr(probe, "MAX_CACHE_ENTRIES", 2)
    probe._CACHE[("old", "a")] = (0.0, {"ok": True})
    probe._CACHE[("newer", "b")] = (250.0, {"ok": True})
    probe._prune_cache(301.0, reserve=1)
    assert ("old", "a") not in probe._CACHE
    assert len(probe._CACHE) <= 1
