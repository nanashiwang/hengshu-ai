# 溯源 · AI API 中转站真伪检测 / Claude · OpenAI · Gemini 中转站真假鉴定工具

[![License: AGPL v3](https://img.shields.io/badge/license-agpl_v3-blue.svg)](LICENSE)
[![Python 3.10+](https://img.shields.io/badge/python-3.10%2B-blue.svg)](https://www.python.org/)
> 品牌：**溯源 / suyuan**　·　口号：**让 AI 能力，有据可验。**

> **溯源 是开源的 AI API 中转站真伪检测工具。**
...
>
> ⭐ 如果 溯源 帮你识别过假中转、避过坑,欢迎给这个仓库点个 Star。
...
部署后通过 `SUYUAN_SITE_URL` 配置公开站点地址。

---

## 支持 溯源

溯源 的检测算法、评分逻辑和报告证据保持开源透明。你可以用两种方式支持项目继续维护:
...
Claude 协议下,启用 thinking 时会返回不透明的 `signature` 字段。仅检查字段存在或长度并不能证明真实性,所以 溯源 会执行两次对照:原始 thinking block 必须被接受,篡改单字节后的 block 必须被服务端以 thinking 校验错误拒绝。只有两项同时成立才给 100 分。它是高强度协议证据,但不是本地公钥验签,也不宣称能绝对排除专门针对探针的恶意中转层。
...
```bash
git clone YOUR_REPOSITORY_URL 溯源
cd 溯源/detector
python3 -m venv venv
...
# 单次连通性测试(秒级,几乎零成本)
./venv/bin/suyuan ping --model claude-haiku-4-5

# 跑完整检测(约 1 分钟,~$0.012)
./venv/bin/suyuan detect \
  --model claude-haiku-4-5 \
  --mode full \
  -o out/test.json

# 跟官方基线对比(自动从 data/baselines/ 找)
./venv/bin/suyuan compare out/test.json
```
...
```bash
suyuan compare <relay_report.json> [-b baseline.json] [-o diff.json]
```
...
```bash
suyuan ping --model claude-haiku-4-5
```
...
├── bench.sh                    # 收集官方基线脚本
├── deploy.sh                   # 部署同步脚本
├── suyuan.service             # systemd unit
├── suyuan-monitor@.service    # 单目标定时检测 systemd 模板
├── suyuan-monitor@.timer      # 默认每小时执行，带随机抖动
│
...
- **目标地址默认隔离**:在线版拒绝本机、内网、链路本地、云元数据和保留地址,避免把检测服务变成 SSRF 跳板。自托管内网检测须显式设置 `SUYUAN_ALLOW_PRIVATE_TARGETS=1`。
...
`curl`。公网部署还必须先准备 `/etc/suyuan.env`，不要把用户提交的中转 key
写入这个文件：

```bash
install -m 0600 /dev/stdin /etc/suyuan.env <<'EOF'
SUYUAN_SITE_URL=https://<your-domain>
SUYUAN_SOURCE_URL=https://<public-source-repository>
SUYUAN_ALLOW_PRIVATE_TARGETS=0
EOF
```
...
`/opt/suyuan-detector`，避免 unit 路径与实际目录悄悄分叉；自定义路径需人工维护对应
...
`deploy/nginx-suyuan.conf.example`：替换域名、启用站点并用 Certbot 配置 HTTPS；
...
`SUYUAN_JOBS_DIR/<protocol>/`，现有历史页和 leaderboard 会直接读取。
目标配置不接收明文 key 参数；systemd 使用隔离 credential 文件。

```bash
# 安装模板
cp suyuan-monitor@.service suyuan-monitor@.timer /etc/systemd/system/
install -d -m 0700 /etc/suyuan-monitor

# 不含密钥的目标配置
cat >/etc/suyuan-monitor/claude-opus.env <<'EOF'
SUYUAN_MONITOR_BASE_URL=https://relay.example
SUYUAN_MONITOR_MODEL='claude-opus-4-8'
SUYUAN_MONITOR_PROTOCOL=anthropic
SUYUAN_MONITOR_MODE=quick
SUYUAN_MONITOR_MAX_CONCURRENT=2
EOF

# 密钥独立保存；LoadCredential 只在任务进程内暴露
install -m 0600 /dev/stdin /etc/suyuan-monitor/claude-opus.key <<'EOF'
sk-replace-me
EOF

systemctl daemon-reload
systemctl enable --now suyuan-monitor@claude-opus.timer
systemctl list-timers 'suyuan-monitor@*'
```

默认每小时执行 quick，并加入 0–10 分钟随机抖动，减少固定时刻探针被针对。
建议另建 daily/weekly 实例执行 standard/full；长上下文默认关闭，只应在单独的
低频实例中显式设置 `SUYUAN_MONITOR_LONG_CONTEXT=1`。

实例级 timer drop-in 可以调整频率，而不复制 unit。下面把 standard 改为每天、
full 改为每 7 天；空赋值用于清除模板继承的每小时设置：

```bash
systemctl edit suyuan-monitor@claude-standard.timer
# 写入：
[Timer]
OnUnitActiveSec=
OnUnitActiveSec=1d
...
systemctl edit suyuan-monitor@claude-full.timer
# 写入：
[Timer]
OnUnitActiveSec=
OnUnitActiveSec=7d
...
systemctl enable --now \
  suyuan-monitor@claude-standard.timer \
  suyuan-monitor@claude-full.timer
```

两个实例应分别使用自己的 `.env` 和 credential 文件，并将
`SUYUAN_MONITOR_MODE` 设置为 `standard` / `full`。不要让多个高成本任务在同一
时间窗口运行。
...
公开部署前请通过 `SUYUAN_SOURCE_URL` 配置当前版本的源码仓库地址。
欢迎 issues / PRs / fork — 完整贡献指南见 [CONTRIBUTING.md](CONTRIBUTING.md)。

## 为什么开源

溯源 的核心交易是「你把 API key 给我,我帮你测中转站真假」。这件事的基础是**信任**:
- 你凭什么相信我们不偷 key?
- 你凭什么相信评分不是收钱定的?
