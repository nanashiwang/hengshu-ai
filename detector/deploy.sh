...
#   XIANCE_HOST=user@host ./deploy.sh

set -euo pipefail

REMOTE_HOST="${XIANCE_HOST:-}"
REMOTE_PATH="${XIANCE_PATH:-/opt/xiance-ai}"
...
Options:
  --host HOST      ssh destination (required unless XIANCE_HOST is set)
...
Environment overrides: XIANCE_HOST, XIANCE_PATH
...
if [[ -z "$REMOTE_HOST" ]]; then
  echo "missing deployment host: pass --host or set XIANCE_HOST" >&2
  exit 2
fi
