...
function signingEnv() {
  ...
  return { HENGSHU_SIGNING_KEY: der.toString('base64') }
}
...
describe('manifest — 确定性 checksum 与 ed25519 签名', () => {
  it('同一 SkillVersion 多次生成 checksum/签名稳定，且不含导出时间戳', () => {
    ...
    const first = buildManifest(skill, version, { siteUrl: 'https://hengshu.example', env })
    const second = buildManifest(skill, version, { siteUrl: 'https://hengshu.example', env })
