...
describe('newapiReconcile — New API 真值与本地 credit 消费漂移检测', () => {
  ...
  it('逐用户漂移报告可导出 JSONL，供运营按 action 人工处理', () => {
    ...
    expect(parsed).toMatchObject({
      schema: 'hengshu.newapi.user_drift.v1',
      userId: 'u1',
      direction: 'newapi_gt_local',
      action: 'manual_backfill_local_or_refund_gateway',
    })
  ...
  it('逐用户漂移报告默认写入 gitignored JSONL，显式路径禁止误覆盖非 JSONL 文件', () => {
    ...
    expect(resolveReconcileDriftReportPath({ cwd: '/tmp/hengshu', monthStart })).toBe(
      '/tmp/hengshu/.reconcile-reports/newapi-drift-2026-07.jsonl',
    )
    expect(() =>
      resolveReconcileDriftReportPath({
        explicitPath: '/tmp/hengshu/.env',
        monthStart,
      }),
  ...
  it('逐用户漂移报告写出后强制 0600，避免用户 ID 与金额被同机用户读取', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'hengshu-drift-'))
  ...
  it('逐用户漂移报告拒绝软链接路径，避免跟随软链覆盖敏感文件', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'hengshu-drift-symlink-'))
  ...
  it('无逐用户漂移时不写空报告', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'hengshu-drift-empty-'))
    await expect(
  ...
  it('漂移人工处理计划输出 JSONL，供运营留痕复核', () => {
    ...
    expect(step).toMatchObject({
      schema: 'hengshu.newapi.user_drift_remediation.v1',
      userId: 'u1',
      suggestedLocalCreditDelta: -30,
    })
