...
beforeAll(async () => {
  ...
  process.env.HENGSHU_SIGNING_KEY = der.toString('base64')
...
describe('signing 往返（信任模型一致性）', () => {
  ...
  it('无签名密钥时优雅降级为 null（不抛错）', async () => {
    vi.resetModules()
    const saved = process.env.HENGSHU_SIGNING_KEY
    delete process.env.HENGSHU_SIGNING_KEY
    const fresh = await import('@/lib/signing')
    expect(fresh.signCanonical({ a: 1 })).toBeNull()
    expect(fresh.getPublicKeyInfo()).toBeNull()
    process.env.HENGSHU_SIGNING_KEY = saved
    vi.resetModules()
  })
})
