...
function validEnv(overrides: Record<string, string | undefined> = {}) {
  return {
    ...
    SERVER_URL: 'https://hengshu.example.com',
    NEXT_PUBLIC_SERVER_URL: 'https://hengshu.example.com',
    ...
    HENGSHU_SIGNING_KEY: signingKey(),
...
describe('productionPreflight — 生产上线配置门禁', () => {
  ...
  it('阻断开发默认密钥、HTTP 域名、缺 Redis/网关/签名私钥', () => {
    ...
    expect(codes).toContain('HENGSHU_SIGNING_KEY_INVALID')
  })
  ...
  it('阻断 SERVER_URL 与 NEXT_PUBLIC_SERVER_URL 不同源，避免 CSRF/CORS 放宽', () => {
    const issues = checkProductionEnv(
      validEnv({
        SERVER_URL: 'https://api.hengshu.example.com',
        NEXT_PUBLIC_SERVER_URL: 'https://hengshu.example.com',
      }),
    )
