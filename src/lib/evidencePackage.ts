...
async function buildPackageFromParts(args: {
  ...
  const core = {
    schemaVersion: 'hengshu.evidence.package/v1',
...
export async function buildPublicSkillEvidencePackage(
  ...
  return { ok: true, filename: safeEvidenceFilename('hengshu-evidence', skill.slug || skill.id), package: pack }
}
...
export async function buildEnterpriseRegistryEvidencePackage(
  ...
  return { ok: true, filename: safeEvidenceFilename('hengshu-enterprise-evidence', registry.id), package: pack }
}
