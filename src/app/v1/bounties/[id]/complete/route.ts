...
// POST /v1/bounties/{id}/complete —— 发布人验收，释放冻结贡献值给接单人
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // 原子结算：状态变更 + 释放贡献值绑进同一事务，任一步失败整体回滚（杜绝“状态已完成但贡献值没到账”）
  ...
  try {
    ...
    if (frozen > 0 && acceptedById) {
      // 释放冻结贡献值（无 'bounty' 规则 → 走传入 points 的可变金额）
  ...
  if (acceptedById) {
    await notify(payload, {
      ...
      title: `悬赏「${b.title}」已验收，${frozen} 贡献值赏金到账`,
      link: `/bounties/${id}`,
      relatedBounty: id,
